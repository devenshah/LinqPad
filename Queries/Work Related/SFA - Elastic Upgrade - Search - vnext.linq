...
private void PerformSearch()
{
    ...
    var searchResponse = client.Search<ApprenticeshipSummary>(s => s
        .Query(q =>
        {
            var query = q.GeoDistance(g => g
                .Field(f => f.Location)
                .Distance(parameters.SearchRadius, DistanceUnit.Kilometers)
                .Location(52, -0.77)
            );
            var keywordQuery = q.Match(f => f.Field(v => v.Title).Query(parameters.Keywords));
            keywordQuery |= q.Match(f => f.Field(v => v.Description).Query(parameters.Keywords));
            keywordQuery |= null;
            
            var qc = new QueryContainer();
            qc = q.Match(f => f.Field(v => v.EmployerName).Query(parameters.Keywords));
            qc |= keywordQuery;
            var qc1 = new QueryContainer();
            return query && qc && qc1;
        }
        ));
