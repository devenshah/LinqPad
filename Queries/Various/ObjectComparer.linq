...
void Main()
{
	var school = new School{ Name = "meschool", NumberOfStudents=100, Address = new Address { FirstLine = "1 oldhouse"}};
	var college = new College{ Name = "meschool", NumberOfStudents=100, Address = new Address()};
	
	
	var properties = school.GetType().GetProperties().Where (p => IsSimpleType(p.PropertyType)).Select (p => p.Name).ToList();
...
//	{
//		
//		var isEqual = ObjectComparer.AreValuesEqual(  
//	}
